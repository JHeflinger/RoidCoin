...
#if defined(NDEBUG)
# error "Roidcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x902b98a26decf447285688826e4d192e2f6c225c98fdd43d65e159ab531f0c23");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Roidcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Roidcoin Signed Message:\n";
...
{
    // Roidcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Roidcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Roidcoin: 3 days
static const int64 nTargetSpacing = 10 * 60; // Roidcoin: 10 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Roidcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Roidcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x20b08230295bcc63fa1ea390ace9f9312d922f03b1a3366ad1c9f455ff210295");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "6/6/2021 - Jason Heflinger has to go to work :(";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0406f8f1ad8aa73869c34a19b5cbc0219ba1431cdfb4ee2b14913e6b23bd1114548de6d7248e35158cd1a7b07229df1a8194ac3b88e0cdb92150864e8d8ff0155b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1623019386;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085065645;

        if (fTestNet)
        {
            block.nTime    = 1623019350;
            block.nNonce   = 385985265;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xad976d01ac4fee0a1cf9cd13e05ac58ddf873fd020aaa6dc130aef066eb2947c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc8, 0xb1, 0xdf }; // Roidcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RoidcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RoidcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RoidcoinMiner(CWallet *pwallet)
{
    printf("RoidcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("roidcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RoidcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RoidcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RoidcoinMiner, pwallet));
}
